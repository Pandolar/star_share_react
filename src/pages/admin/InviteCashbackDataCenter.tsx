  ...
  return <Card><CardBody className="space-y-5">
    ...
    {!campaignId ? <div className="py-8 text-center text-default-500">请先创建返现活动</div> : loading && !overview ? <div className="py-8 text-center"><Spinner label="加载活动数据..." /></div> : <>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric label="可用 / 支付提现预占" value={`¥${Number(summary?.available_amount || 0).toFixed(2)} / ¥${Number(summary?.reserved_amount || 0).toFixed(2)}`} />
        <Metric label="累计抵扣 / 已提现" value={`¥${Number(summary?.purchase_consumed_amount || 0).toFixed(2)} / ¥${Number(summary?.withdraw_done_amount || 0).toFixed(2)}`} />
      </div>
