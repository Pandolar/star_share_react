...
const OrdersManagePage: React.FC = () => {
  ...
  const statusOptions = [
    ...
    const renderInvoiceStatus = (order: Order) => {
        ...
        const labels: Record<string, string> = { not_requested: '未申请', awaiting_payment: '待支付', pending_issue: '待开票', processing: '开票中', issued: '已开票', cancelled: '已取消', payment_exception: '异常到账' };
        const color = order.invoice_status === 'issued' ? 'success' : order.invoice_status === 'processing' ? 'primary' : order.invoice_status === 'pending_issue' ? 'warning' : 'default';
